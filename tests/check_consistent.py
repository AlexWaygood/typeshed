...
# Dependencies in these dependency groups must have exact versions in the requirements file to ensure
# consistent CI runs.
dependency_groups_requiring_exact_pins = {"lint", "mypy", "pytype"}
...
def get_precommit_requirements() -> list[Requirement]:
    ...
    precommit_requirements: list[Requirement] = []
    for repo in yam["repos"]:
        ...
        precommit_requirements.append(Requirement(f"{package_name}=={package_rev}"))
        precommit_requirements.extend(Requirement(additional_req) for additional_req in hook.get("additional_dependencies", []))
...
def check_requirement_pins() -> None:
    ...
    all_requirements = get_pyproject_requirements()
    relevant_requirements = chain.from_iterable(
        all_requirements[group_name] for group_name in dependency_groups_requiring_exact_pins
    )
    for requirement in relevant_requirements:
        name, spec = requirement.name, requirement.specifier
        assert len(spec) == 1, f"type checker/linter {name!r} has complex specifier in pyproject.toml"
        msg = f"type checker/linter {name!r} is not pinned to an exact version in pyproject.toml"
        assert next(iter(spec)).operator == "==", msg


def check_precommit_requirements() -> None:
    """Check that the requirements in pyproject.toml and .pre-commit-config.yaml match."""
    merged_pyproject_requirements = {
        requirement.name: requirement.specifier for requirement in chain.from_iterable(get_pyproject_requirements().values())
    }
    precommit_requirements = get_precommit_requirements()
    no_pyproject_entry_msg = "All pre-commit requirements must also be listed in `pyproject.toml` (missing {name!r})"
    for requirement in precommit_requirements:
        name, specifier = requirement.name, requirement.specifier
        assert name in merged_pyproject_requirements, no_pyproject_entry_msg.format(name=name)
        specifier_mismatch = (
            f"Specifier '{specifier}' for {name!r} in `.pre-commit-config.yaml` "
            f"does not match specifier '{merged_pyproject_requirements[name]}' in `pyproject.toml`"
        )
        assert specifier == merged_pyproject_requirements[name], specifier_mismatch
